 ...
 * are pre-populated with resized WebP variants generated in the admin's browser at upload time
 * (see lib/client-image-processing.ts) — no paid Cloudflare Images/Image Resizing subscription
 * needed, and no image-codec CPU work ever runs in the deployed Worker. This loader just appends
 * the requested width so the origin route can pick the smallest variant that still covers it.
 * Static public assets (logo, hero fallback, category stills) have no variants and are served
 * as-is.
 */
